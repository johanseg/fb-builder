...
- **Context providers** (4):
  - `AuthContext` — JWT state, `login()`, `logout()`, `authFetch()` (auto-refreshes on 401), `hasRole()`, `hasPermission()`
...
- File uploads: images (`jpg/jpeg/png/gif/webp`) up to 10MB and video (`mp4/mov/avi/webm`) up to 500MB, UUID filenames.
- Public self-signup is disabled; admins create users through the authenticated backend flow.
